...
if(isset($_GET['id'])){
	$id=$_GET['id'];
	$query_lb="SELECT b.*,l.tenloai from bongden b LEFT JOIN loaibd l 
	ON b.maloai=l.maloai WHERE b.maloai=".$id." ORDER BY b.khoiluong ASC limit 14;";
} else {
	$query_lb="SELECT b.*,l.tenloai from bongden b LEFT JOIN loaibd l 
	ON b.maloai=l.maloai WHERE 1 ORDER BY b.khoiluong ASC limit 14;";
}
						...
						<h4><i class="glyphicon glyphicon-minus fas fa-sliders-h" aria-hidden="true"></i>Thương hiệu</h4>
							<div class="row row1 scroll-pane">
								...
								<?php foreach ($menu_ths as $menu_th) 
									...
									<label class="checkbox"><input type="checkbox" name="checkbox"><i></i>Gold Medal</label>checked="" -->
								</div>
							</div>
				   </section>				   
		</div>
